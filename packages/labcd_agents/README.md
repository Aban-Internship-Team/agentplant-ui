...
Shared, reusable foundation for LLM-backed agents in this repo — providers,
retries, token/cost tracking, prompt loading, and JSON-response parsing.

Public API focuses on `LLMFactory`, message helpers, prompt loading, pricing,
and token accounting so AgentPlant (and any future agents) share one stack.
